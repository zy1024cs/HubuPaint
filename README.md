...
Design of Windows-like Sketchpad Based on. net
Abstract: With the continuous development of society and the continuous popularization of computers, people have higher and higher requirements for drawing and higher and more comprehensive requirements for drawing functions. People of different ages and different identities have different needs. At present, ordinary users generally use the Windows drawing tool that comes with Windows system. It can realize the most basic drawing function, but it is not flexible enough to use and its function is not perfect. Based on this, this design attempts to design a WinForm wi ndowdrawing software based on. Net Framework with more perfect functions and more flexible use according to the design idea of Windows drawing tools and considering various factors comprehensively. It not only can meet the needs of daily drawing, but also has the advantages of convenient storage, editing and display, low power consumption, easy expansion and humanization of the interface.

Keyword: c#  .Net Framework  WinForm  Drawing software
...
（4）将剪切板上的图形在画板上画出并将其保存到shape_push栈里面。
3.2.5 核心操作设计
